import { getAllPages } from "@/actions/customPage";
import PagesListClient from "./PagesListClient";
import { Layers } from "lucide-react";
import Link from "next/link";
import { Button } from "@/components/ui/button";

export default async function PagesPage() {
  const pages = await getAllPages();

  return (
    <div className="space-y-6 animate-slide-up max-w-6xl mx-auto">
      <div className="flex justify-between items-center bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-white/5 transition-colors">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 bg-purple-100 dark:bg-purple-500/20 text-purple-600 dark:text-purple-400 rounded-xl flex items-center justify-center ring-4 ring-purple-50 dark:ring-purple-500/10 shrink-0">
             <Layers className="w-6 h-6" />
          </div>
          <div>
            <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Custom Pages</h1>
            <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
              Kelola halaman statis ekstra (profil, fasilitas, dll) menggunakan Drag and Drop Builder.
            </p>
          </div>
        </div>
      </div>

      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-white/5 transition-colors p-6">
        <PagesListClient initialPages={pages} />
      </div>
    </div>
  );
}
